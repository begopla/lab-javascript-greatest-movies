...
function getAllDirectors(aMovie) { 
  ...
  //Bonus 1.1
  // const allOrderedDirectors =allDirectors.sort()
   
  // const filteredDirectors = allOrderedDirectors.filter((item,index) => allOrderedDirectors.indexOf(item)===index);
...
function dramaMoviesScore(aMovie) {

const genreArray = aMovie.filter((movie) => { 
  ...
  if(genreArray.length===0) return 0;
   ...
   //Extract scores to an Array
   const scoreArray = genreArray.map((item =>{return item.score}))
  
   // Calculate Average
   const totalScore= scoreArray.reduce((acuValue, value) => {
  return acuValue +value;},0)
   const averageValue = totalScore/scoreArray.length
...
const titleArray = aMovie.map(item=>{return item.title})
...
if (aMovie.length===0){
  ...
  //   let averageScore;
  //   const modifiedScoreToAverage= sortedArray.map(x=>{
  //     if(sortedArray[i].year===sortedArray[i+1].year){
  //       averageScore=(averageScore+((sortedArray[i].score+sortedArray[i+1])/2))/2
  //     }
  //     return x.score=averageScore})
  //     }
    
  // return modifiedScoreToAverage
